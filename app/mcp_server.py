...
from app.core.config import REPORTER_KIND_HEADER, settings
...
def _source_from(ctx: Context | None) -> str:
    """Honour an ``X-Reporter-Kind: demo`` header sent by the MCP client.

    Demo agents label themselves so their traffic stays out of the real
    adoption metrics while still being usable evidence.
    """
    return source_from_kind(_request_headers(ctx).get(REPORTER_KIND_HEADER.lower()))
...
async def check_tool_failure(
    service: Annotated[
        str,
        Field(description="Tool/service identifier, e.g. 'github-mcp'.", max_length=128),
    ],
    operation: Annotated[
        str,
        Field(
            description="Operation/tool name that failed, e.g. 'create_issue'.",
            max_length=128,
        ),
    ],
...
async def report_tool_failure(
    service: Annotated[str, Field(description="Tool/service identifier.", max_length=128)],
    operation: Annotated[str, Field(description="Operation that failed.", max_length=128)],
    error_type: Annotated[
        str | None,
...
async def report_tool_success(
    service: Annotated[str, Field(description="Tool/service identifier.", max_length=128)],
    operation: Annotated[
        str, Field(description="Operation that succeeded.", max_length=128)
    ],
    version: Annotated[
        str | None, Field(description="Version of the service/tool.", max_length=64)
    ] = None,
