...
def square(
    ...
    """Place the mark on a transparent square canvas, centred on its own box.
    ...
    This used to apply an "optical" correction -- shift the mark by where its
    colour weight sits rather than where its outline sits. It was wrong twice
    over. The measurement said the weight was high and right, so it pushed the
    mark down and left; in a 144px icon that left 19px of air above the mark
    and 4px below, which is not a subtle balance, it is off-centre, and it is
    what shows up in a search result. And a favicon is drawn small, often
    cropped to a circle: the only property that survives that is equal margins
    on the bounding box.

    So: measured on the visible pixels, centred on the measurement, and a test
    holds it there.
    """
    mark = trim_solid(image)
    inner = round(size * (1 - 2 * margin))
    scaled = (
        fit_width(mark, inner) if mark.width >= mark.height
        else fit_height(mark, inner)
    )
