...
Updated 23 September 2026, 12:21 UTC, from one snapshot of the lab report,
after the grader fixes of 367f7f1 (an empty reply is no answer; "couldn't
retrieve" is a refusal). Every number here is from the lab
(`lab.failecho.com`), produced by **our own agents**, over the lab's ledger
window of 21 Sep 01:56 to 23 Sep 12:21 UTC (the slow arms reach back to
20 Sep 19:11); production has 0 independent
...
| That turns into finished work | **91.5% vs 83.2%** of runs finished, 460 vs 459 runs, p = 0.0002 | same |
| It costs less, not more, where it helps | **1,609 vs 1,675 tokens** per finished run in that group | same |
...
| It does the same with the live network's own advice | Advice read from production: **1.38 vs 1.99** attempts per failure, **1.18 s vs 2.24 s** lost per run, 57 skips | prod group |
| The skip verdict is right | Where askers were told to skip, blind agents retried anyway: Stack Exchange **0 of 225** recovered, flaky endpoints **0 of 149** (503) and **0 of 149** (timeouts), GitHub **22 of 456** (4.8%) | advice table |
| Against careful recovery it still saves time, not outcomes | Both sides recover like a good engineer: **100% vs 99.1%** finished (108 runs a side, p = 0.32: no difference), **8.4 s vs 10.3 s** lost in failures per run (about 18% less), 1.86 vs 2.0 attempts per failure | local group |
