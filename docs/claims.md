...
| It does the same with the live network's own advice | Advice read from production: **1.40 vs 1.99** attempts per failure (p < 0.0001), **1.07 s vs 2.13 s** lost per run (126 runs a side, p = 0.014), 61 skips | prod group |
