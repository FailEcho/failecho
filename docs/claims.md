...
| "Private mode works with every FailEcho integration" | npm `failecho-mcp` is still 0.2.2 (checked 23 Sep 11:25): `npx failecho-mcp proxy` does not read `FAILECHO_TEAM` and a team using it reports **publicly**. Everything else works: REST, Claude Code plugin 0.2.0+, OpenCode plugin (file or npm), `failecho-autoreport` 0.1.7+, the PyPI proxy 0.2.3+ |
